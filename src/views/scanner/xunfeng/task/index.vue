<template>
  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="插件" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <span class="link-type">{{ scope.row.plugins | filterPlugin }}</span>
        </template>
      </el-table-column>
      <el-table-column label="目标" align="center" show-overflow-tooltip>
        <template slot-scope="scope">
          <span class="link-type">{{ scope.row.targets| filterTarget }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" prop="created_at" label="添加时间" show-overflow-tooltip>
        <template slot-scope="scope">
          <i class="el-icon-time"/>
          <span>{{ scope.row.add_time | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="created_at" label="上一次完成时间" show-overflow-tooltip>
        <template slot-scope="scope">
          <i class="el-icon-time"/>
    ...
    <el-dialog title="新增任务" :visible.sync="dialogFormVisible">
      ...
      >
        <el-form-item label="定时任务" prop="title">
        ...
        <div style="display: flex;margin-bottom: 40px">
          <label class="typo__label" style="margin-right: 100px;display: inline" for="ajax">主机</label>
          <multiselect
            id="ajax_host"
            v-model="host_value"
            label="ip"
...
<script>
  ...
  export default {
    ...
    filters: {
      parseTime: parseTime,
      filterPlugin: function (val) {
        let arr = new Array(val)
        return arr.join(',')
      },
      filterTarget: function (val) {
        let arr = val
        return arr.map(function (item) {
          return item.ip+'['+item.port+':'+item.service+']'
        }).join(',')
      }
    },
    data() {
    ...
    methods: {
      ...
      postTask() {
        let pluginList = []
        let hostList = []
        this.plugin_value.forEach(function (plugin) {
          pluginList.push(plugin.name)
        })
        this.host_value.forEach(function (host) {
          hostList.push({
            ip: host.value.split('|')[0],
            port: host.value.split('|')[1],
            service: host.value.split('|')[2] || ''
          })
        })
        const data = {
